...
# Добавляем путь к директории src, чтобы работали импорты
sys.path.append(os.path.join(os.path.dirname(__file__), "src"))

# Импорты модулей из src/
from api.openrouter import OpenRouterClient  # Клиент для работы с API OpenRouter
from ui.components import MessageBubble  # Компонент для отображения сообщений
